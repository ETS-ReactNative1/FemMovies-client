...
function MovieCard(props) {
  ...
  const removeFav = (movie) => {
    axios.delete(`https://femmovies.herokuapp.com/users/${user.Username}/movies/${movie._id}`)
      .then(() => {
  ...
  const addFav = (movie) => {
    axios.post(`https://femmovies.herokuapp.com/users/${user.Username}/movies/${movie._id}`)
      .then(() => {
        // Change state of favoriteMovies to rerender component
